...
**Banner lifecycle (1.0.8).** `attachAdView` destroys the AdView a frame previously held and binds pause/resume/destroy to the host Activity's lifecycle. Before that, each reload only *detached* the old AdView — which keeps its own refresh timer — so every reload left another invisible AdView requesting ads forever, and the visible one kept refreshing while its screen was paused.
